...
Vector BotBodyTarget (edict_t *pBotEnemy, bot_t *pBot)
{
   ...
   if (pBot->iStates & STATE_SUSPECTENEMY)
   {
      ...
      target.x = target.x + RANDOM_FLOAT (-40.0, 40.0);
      target.y = target.y + RANDOM_FLOAT (-40.0, 40.0);
      target.z = target.z + RANDOM_FLOAT (-50.0, 50.0);
   }
   else
   {
      if ((ucVis & HEAD_VISIBLE) && (ucVis & WAIST_VISIBLE))
      {
         if (RANDOM_LONG (1, 100) < BotAimTab[pBot->bot_skill / 20].iHeadShot_Frequency)
            ...
            target = pBotEnemy->v.origin + (pBotEnemy->v.view_ofs / 4);  // aim for body
      }
   ...
   else if ((pEdict->v.fov == 40) || (pEdict->v.fov == 55))
      fOffset = fDistance / 2000;
   else
      fOffset = fDistance / 1000;
   target.x += RANDOM_FLOAT (-(fOffset * BotAimTab[pBot->bot_skill / 20].fAim_X), (fOffset * BotAimTab[pBot->bot_skill / 20].fAim_X));
   target.y += RANDOM_FLOAT (-(fOffset * BotAimTab[pBot->bot_skill / 20].fAim_Y), (fOffset * BotAimTab[pBot->bot_skill / 20].fAim_Y));
   target.z += RANDOM_FLOAT (-(fOffset * BotAimTab[pBot->bot_skill / 20].fAim_Z), (fOffset * BotAimTab[pBot->bot_skill / 20].fAim_Z));
...
bool FireHurtsFriend (bot_t *pBot, float fDistance)
{
   ...
   for (i = 0; i < gpGlobals->maxClients; i++)
   {
      ...
      if (GetShootingConeDeviation (pEdict, &pPlayer->v.origin) > 0.90)
      {
...
void BotDoAttackMovement (bot_t *pBot)
{
   ...
   if (pBot->fTimeWaypointMove + pBot->fTimeFrameInterval + 0.5 < gpGlobals->time)
   {
      ...
      if ((pBot->bot_skill > 60) && (pBot->byFightStyle == 0))
      {
         ...
         if (pBot->bot_skill > 80)
         {
            if ((pBot->f_jumptime + 2.0 < gpGlobals->time) && (pEdict->v.flags & FL_ONGROUND))
            {
