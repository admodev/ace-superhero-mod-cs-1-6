...
C_DLLEXPORT int Meta_Attach (PLUG_LOADTIME now, META_FUNCTIONS *pFunctionTable, meta_globals_t *pMGlobals, gamedll_funcs_t *pGamedllFuncs)
{
   ...
   REG_SVR_COMMAND (CONSOLE_CMD_POD, PODBot_ServerCommand);
...
void ClientDisconnect (edict_t *pEntity)
{
   ...
   else if (num_players < max_bots)
   {
      ...
      if (index < gpGlobals->maxClients)
      {
         ...
         BotCreateTab[index].bot_skill = 101;
         BotCreateTab[index].bot_personality = 5;
         BotCreateTab[index].bot_team = 5;
         BotCreateTab[index].bot_class = 5;
         BotCreateTab[index].bNeedsCreation = TRUE;
         if (botcreation_time == 0.0)
            botcreation_time = gpGlobals->time;
      }
      else
         UTIL_ServerPrint ("Bot creation queue full!\n");
   }
...
void ClientCommand (edict_t *pEntity)
{
   ...
   if (!isFakeClientCommand && (pEntity == pHostEdict) && !g_bIsDedicatedServer)
   {
      ...
      if (FStrEq (pcmd, CONSOLE_CMD_POD))
      {
         PODBot_ServerCommand();
      ...
      else if (pUserMenu != NULL)
      {
         if (FStrEq (pcmd, "menuselect"))
         {
            ...
            else if (pUserMenu == &menuPODBotMain)
            {
               ...
               if (FStrEq (arg1, "1"))
               {
                  ...
                  if (i < gpGlobals->maxClients)
                  {
                     ...
                     UTIL_HostPrint ("Bot creation queue full!\n");
               }
               else if (FStrEq (arg1, "2"))
                  ...
                  UTIL_ShowMenu (&menuPODBotFillServerSkill);
               else if (FStrEq (arg1, "6"))
                  UTIL_ShowMenu (&menuPODBotKickBotTeam);
               else if (FStrEq (arg1, "7"))
                  UserRemoveAllBots ();
               else if (FStrEq (arg1, "8"))
            ...
            else if (pUserMenu == &menuPODBotAddBotSkill)
            {
               ...
               if (FStrEq (arg1, "1"))
               {
                  iStoreAddbotSkill = RANDOM_LONG (0, 19);
                  UTIL_ShowMenu (&menuPODBotAddBotPersonality);
               }
               else if (FStrEq (arg1, "2"))
               {
                  iStoreAddbotSkill = RANDOM_LONG (20, 39);
                  UTIL_ShowMenu (&menuPODBotAddBotPersonality);
               }
               else if (FStrEq (arg1, "3"))
               {
                  iStoreAddbotSkill = RANDOM_LONG (40, 59);
                  UTIL_ShowMenu (&menuPODBotAddBotPersonality);
               }
               else if (FStrEq (arg1, "4"))
               {
                  iStoreAddbotSkill = RANDOM_LONG (60, 79);
                  UTIL_ShowMenu (&menuPODBotAddBotPersonality);
               }
               else if (FStrEq (arg1, "5"))
               {
            ...
            else if (pUserMenu == &menuPODBotAddBotPersonality)
            {
               UTIL_ShowMenu (NULL); // reset menu display

               if (FStrEq (arg1, "1"))
               {
                  iStoreAddbotPersonality = 0;
                  UTIL_ShowMenu (&menuPODBotAddBotTeam);
               }
               else if (FStrEq (arg1, "2"))
               {
                  iStoreAddbotPersonality = 1;
                  UTIL_ShowMenu (&menuPODBotAddBotTeam);
               }
               else if (FStrEq (arg1, "3"))
               {
                  iStoreAddbotPersonality = 2;
                  UTIL_ShowMenu (&menuPODBotAddBotTeam);
               }
               else if (FStrEq (arg1, "5"))
               {
                  iStoreAddbotPersonality = 5;
                  UTIL_ShowMenu (&menuPODBotAddBotTeam);
               }
            ...
            else if (pUserMenu == &menuPODBotAddBotTeam)
            {
               ...
               else if (FStrEq (arg1, "5"))
               {
                  ...
                  if (i < gpGlobals->maxClients)
                  {
                     memset (&BotCreateTab[i], 0, sizeof (createbot_t));
                     BotCreateTab[i].bNeedsCreation = TRUE;
                     BotCreateTab[i].bot_skill = iStoreAddbotSkill;
                     BotCreateTab[i].bot_personality = iStoreAddbotPersonality;
                     BotCreateTab[i].bot_team = iStoreAddbotTeam;
                     BotCreateTab[i].bot_class = 5;
                  }
                  else
                     UTIL_HostPrint ("Bot creation queue full!\n");
                     ...
                     || (pUserMenu == &menuPODBotAddBotCTModel))
            {
               ...
               if ((atoi (arg1) >= 1) && (atoi (arg1) <= 5))
               {
                  ...
                  if (i < gpGlobals->maxClients)
                  {
                     ...
                     BotCreateTab[i].bot_personality = iStoreAddbotPersonality;
                     BotCreateTab[i].bot_team = iStoreAddbotTeam;
                     BotCreateTab[i].bot_class = atoi (arg1);
                  }
                  else
                     UTIL_HostPrint ("Bot creation queue full!\n");
            ...
            else if (pUserMenu == &menuPODBotFillServerSkill)
            {
               UTIL_ShowMenu (NULL); // reset menu display

               if (FStrEq (arg1, "1"))
               {
                  iStoreFillServerSkill = RANDOM_LONG (0, 19);
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "2"))
               {
                  iStoreFillServerSkill = RANDOM_LONG (20, 39);
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "3"))
               {
                  iStoreFillServerSkill = RANDOM_LONG (40, 59);
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "4"))
               {
                  iStoreFillServerSkill = RANDOM_LONG (60, 79);
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "5"))
               {
                  iStoreFillServerSkill = RANDOM_LONG (80, 99);
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "6"))
               {
                  iStoreFillServerSkill = 100;
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
               else if (FStrEq (arg1, "7"))
               {
                  iStoreFillServerSkill = 101;
                  UTIL_ShowMenu (&menuPODBotFillServerPersonality);
               }
            ...
            else if (pUserMenu == &menuPODBotFillServerPersonality)
            {
               UTIL_ShowMenu (NULL); // reset menu display

               if (FStrEq (arg1, "1"))
               {
                  iStoreFillServerPersonality = 0;
                  UTIL_ShowMenu (&menuPODBotFillServerTeam);
               }
               else if (FStrEq (arg1, "2"))
               {
                  iStoreFillServerPersonality = 1;
                  UTIL_ShowMenu (&menuPODBotFillServerTeam);
               }
               else if (FStrEq (arg1, "3"))
               {
                  iStoreFillServerPersonality = 2;
                  UTIL_ShowMenu (&menuPODBotFillServerTeam);
               }
               else if (FStrEq (arg1, "5"))
               {
                  iStoreFillServerPersonality = 5;
                  UTIL_ShowMenu (&menuPODBotFillServerTeam);
               }
            ...
            else if (pUserMenu == &menuPODBotFillServerTeam)
            {
               UTIL_ShowMenu (NULL); // reset menu display

               if (FStrEq (arg1, "1"))
               {
                  iStoreFillServerTeam = 1;
                  UTIL_ShowMenu (&menuPODBotFillServerTModel);
               }
               else if (FStrEq (arg1, "2"))
               {
                  iStoreFillServerTeam = 2;
                  UTIL_ShowMenu (&menuPODBotFillServerCTModel);
               }
               else if (FStrEq (arg1, "5"))
               {
                  for (i = 0; i < gpGlobals->maxClients; i++)
                     if (!BotCreateTab[i].bNeedsCreation)
                     {
                        memset (&BotCreateTab[i], 0, sizeof (createbot_t));
                        BotCreateTab[i].bNeedsCreation = TRUE;
                        BotCreateTab[i].bot_skill = iStoreFillServerSkill;
                        BotCreateTab[i].bot_personality = iStoreFillServerPersonality;
            ...
            else if ((pUserMenu == &menuPODBotFillServerTModel)
                     || (pUserMenu == &menuPODBotFillServerCTModel))
            {
               UTIL_ShowMenu (NULL); // reset menu display
               ...
               if ((atoi (arg1) >= 1) && (atoi (arg1) <= 5))
               {
                  ...
                  for (i = 0; i < gpGlobals->maxClients; i++)
                     if (!BotCreateTab[i].bNeedsCreation)
                     {
                        ...
                        BotCreateTab[i].bot_skill = iStoreFillServerSkill;
                        BotCreateTab[i].bot_personality = iStoreFillServerPersonality;
                        BotCreateTab[i].bot_team = iStoreFillServerTeam;
                        BotCreateTab[i].bot_class = atoi (arg1);
                     }
            ...
            // Kick Bot Team Select Menu ?
            else if (pUserMenu == &menuPODBotKickBotTeam)
            {
               UTIL_ShowMenu (NULL); // reset menu display
               ...
               {
                  for (i = 0; i < gpGlobals->maxClients; i++)
                     if (bots[i].is_used && !FNullEnt (bots[i].pEdict) && (bots[i].bot_team == TEAM_CS_TERRORIST))  // is this slot used? Terrorist?
                     {
               ...
               {
                  for (i = 0; i < gpGlobals->maxClients; i++)
                     if (bots[i].is_used && !FNullEnt (bots[i].pEdict) && (bots[i].bot_team == TEAM_CS_COUNTER))  // is this slot used? Counter-Terrorist?
                     {
                        sprintf (kick_command, "kick \"%s\"\n", STRING (bots[i].pEdict->v.netname));
                        SERVER_COMMAND (kick_command); // kick the bot using (kick "name")
                        break;
   ...
   if (g_GameRules)
   {
      ...
      while ((bot_cfg_fp != NULL) && !feof (bot_cfg_fp) && (pause_time < gpGlobals->time))
      {
         if (fgets (cmd_line, 1024, bot_cfg_fp) != NULL)
         {
            ...
            sprintf (arg5, GetField (cmd_line, 5));
            ...
            else if (FStrEq (cmd, CONSOLE_CMD_ADDBOT))
            {
               ...
               // add bots to bot creation queue if queue is empty
               if (bot_index < gpGlobals->maxClients)
               {
                  memset (&BotCreateTab[bot_index], 0, sizeof (createbot_t));
                  strcpy (BotCreateTab[bot_index].bot_name, arg5);
                  BotCreateTab[bot_index].bot_skill = atoi (arg1);
                  BotCreateTab[bot_index].bot_personality = atoi (arg2);
                  BotCreateTab[bot_index].bot_team = atoi (arg3);
                  BotCreateTab[bot_index].bot_class = atoi (arg4);
...
void PODBot_ServerCommand (void)
{
   ...
   sprintf (arg2, CMD_ARGV (3));
   sprintf (arg3, CMD_ARGV (4));
   sprintf (arg4, CMD_ARGV (5));
   sprintf (arg5, CMD_ARGV (6));

   // "addbot" adds a bot to creation queue with specified args
   if (FStrEq (pcmd, CONSOLE_CMD_ADDBOT))
   {
      ...
      {
         memset (&BotCreateTab[index], 0, sizeof (createbot_t));
         strcpy (BotCreateTab[index].bot_name, arg5);
         BotCreateTab[index].bot_skill = atoi (arg1);
         if ((arg2 != NULL) && (*arg2 != 0))
            BotCreateTab[index].bot_personality = atoi (arg2);
         else
            BotCreateTab[index].bot_personality = 5;
         BotCreateTab[index].bot_team = atoi (arg3);
         BotCreateTab[index].bot_class = atoi (arg4);
         BotCreateTab[index].bNeedsCreation = TRUE;
         if (botcreation_time == 0.0)
            botcreation_time = gpGlobals->time;
      }
   ...
   // "jasonmode" Forces bots to use knives if on
   ...
   // If it were none of the above commands tell the user
      ...
      UTIL_HostPrint ("\"%s\" is not a Podbot command\n", pcmd);
}
